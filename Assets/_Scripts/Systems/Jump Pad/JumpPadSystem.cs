...
public partial class JumpPadSystem : SystemBase
{
    ...
    {
        RequireForUpdate<JumpPad>();
    }
    protected override void OnUpdate()
    {
        // Iterate on all jump pads with trigger event buffers
        foreach (var (jumpPad, triggerEventsBuffer, transform, entity) in SystemAPI.Query<JumpPad, DynamicBuffer<StatefulTriggerEvent>, RefRO<LocalTransform>>().WithEntityAccess())
        {
            ...
            for (int i = 0; i < triggerEventsBuffer.Length; i++)
            {
                ...
                if (triggerEvent.State == StatefulEventState.Enter && SystemAPI.HasComponent<KinematicCharacterBody>(otherEntity))
                {
                    ...
                    characterBody.RelativeVelocity += math.mul(transform.ValueRO.Rotation, jumpPad.JumpForce);
